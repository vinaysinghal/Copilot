...
.DESCRIPTION
    Picks message from azure SQL DB and process it.
...
Function ScriptError {
    ...
    # Remove DB update
    try {
        # Update DB entry with error status and message (Canceled = 5)
        Invoke-Sqlquery -qry "UPDATE Licensing_Dev.License_Requests SET StatusID = 2, Comments = ISNULL(Comments + ' | ', '') + 'Error: $msg', UpdatedBy = 'DW-Automation' WHERE ID = $id;"
        Write-Log 'DB entry updated with error status' -Level 'INFO'
        Write-Log "**********************************************************************"
        continue
    }
    catch {
        ...
        Write-Log 'Failed to update DB entry during scriptError.' -Level 'ERROR'
        Write-Log "**********************************************************************"
        continue
    }
}
...
function Get-SOUStatus {
    ...
    if($null -ne $LAppCaseAssignedDate){
        ...
        if ($LAppCaseAssignedDate -lt $TwentyEightDaysAgo) {
            ...
            Invoke-Sqlquery -qry "UPDATE Licensing_Dev.License_Requests SET StatusID = 6, CompletionDate=GETUTCDATE(), UpdatedBy = 'DW-Automation', Comments = ISNULL(Comments + ' | ', '') + 'Training expired' WHERE ID = $id;"
            ...
            if($null -ne $UserPTWData){
                if(($UserPTWData.'Training record status' -notcontains 'Completed') -and ($UserSOUData.'Training record status' -eq 'Completed')){
                    ...
                    Write-Log "Message with UPN $userUPN - $LicenseType $Action updated in DB."
                    ...
                    try {
                        $UpdateTicket = Update-TicketStatus -TicketNumber $RITMNumber -State '3' -Stage 'Training Expired' -WorkNotes "User's Passport To Work training record shows as incomplete. Microsoft 365 Copilot license request rejected. Closing the ticket."
...
function Invoke-UpgradeToCopilot {
    ...
    elseif ($SOUTrainingStatus -eq 'Pending') {
        Write-Log "User is yet to complete the SOU training or accept TOU. Awaiting completion report..." -Level 'INFO'
        $LicenseAssignmentStatus = 'Pending'
        Write-Log "**********************************************************************" -Level 'INFO'
        continue
    }
    elseif ($SOUTrainingStatus -eq 'Failed') {
        Write-Log 'User has failed the SOU training or declined SOU. Check CSOD report for further clarification.' -Level 'INFO'
...
foreach ($messageString in $dbmessages) {
    ...
    $LAppCaseAssignedDate = $messageString.LAppCaseAssignedDate
    ...
    if($RITMNumber -ne 'RITM000002847418'){
        continue
    }
    ...
    try{
        ...
        if($msg -like '*does not exist*'){
            ...
            try {
                Invoke-Sqlquery -qry "UPDATE Licensing_Dev.License_Requests SET StatusID = 5, UpdatedBy = 'DW-Automation', Comments = ISNULL(Comments + ' | ', '') + 'Invalid user in Entra' WHERE ID = $id;"
                Write-Log "***********************************************************************" -Level 'INFO'
                continue
            }
            catch {
                $ex = $_.Exception.Message
                Write-Log $ex -Level 'ERROR'
                Write-Log "Failed to update DB for invalid user in Entra."
                continue
            }
        }else{
    ...
    #Update DB if request is completed (was Dataverse)
    if ($RequestCompletionStatus -eq $true) {
        ...
        try {
            write-log 'Updating message in DB...' -Level 'INFO'
            ...
            write-log 'Record updated in DB...' -Level 'INFO'
            Write-Log "**********************************************************************" -Level 'INFO'
            continue
        }
        catch {
            $ex = $_.Exception.Message
            Write-Log $ex -Level 'ERROR'
            Write-Log "Failed to update DB for license replicated."
            continue
        }
    }
    ...
    if ($action -eq 'upgrade') {
        if ($LicenseType -eq "MicrosoftCopilot") {
            ...
            try {
                ...
                if ($RITMStatus) {
                    ...
                    if (($statecode -contains $CurrentState)) {
                        ...
                        try {
                            write-log 'Updating record in database...'
            ...
            $LappCaseNumber = $null  
            # Only check if overall Copilot licenses are available and cutoff not reached
            if (($availableCopilotLicenses -gt 0) -and ($ThisRunProcessed -lt $ProcessCutoff)) {
                ...
                if($UserExtensionAttribute1 -eq 'Supply, Trading & Shipping'){
                    if ([string]::IsNullOrEmpty($LAppCase) -or ($LAppCase -like '*Invalid NTIDUser1*')) {
                        if([string]::IsNullOrEmpty($LAppCase)){
                        ...
                        if($LAppCase -like '*Invalid NTIDUser1*'){
                            Write-Log "Invalid NTIDUser1 in LApp case response. Updating the record." -Level 'ERROR'
                            try {
                                Invoke-Sqlquery -qry "UPDATE Licensing_Dev.License_Requests SET StatusID = 2, UpdatedBy = 'DW-Automation', LAppCase = NULL, LAppCaseAssignedDate = NULL, Comments = ISNULL(Comments + ' | ', '') + 'Removing Invalid NTIDUser1 in LApp case response' WHERE ID = $id;"
                                Write-Log "Request closed due to invalid NTIDUser1 in LApp case response." -Level 'INFO'
                            }
                            catch {
                                $ex = $_.Exception.Message
                                Write-Log $ex -Level 'ERROR'
                                Write-Log "Failed to update DB for invalid NTIDUser1."
                            }
                        }
                        ...
                        Write-Log 'Updating the database record with Lapp case number ... ' -Level 'INFO'
                    ...
                    Write-Log "User $userUPN is part of T&S entity. Proceeding with SOU and PTW training status check." -Level 'INFO'
                    try {
                    ...
                    Write-Log "User $userUPN is not part of T&S entity. Proceeding with TOU acceptance status check." -Level 'INFO'
                    try {
                ...
                if ($CopilotAssignmentStatus -contains 'Assigned') {
                    ...
                    write-log 'Copilot license assigned to the user. Updating record in DB...'
                    try {
                        ...
                        $ex = $_.Exception.Message
                        Write-Log $ex -Level 'ERROR'
                        ScriptError('Failed to update record in database.')
                    }
                ...
                elseif ($CopilotAssignmentStatus -contains 'Failed') {
                    write-log 'Copilot license declined. Updating record in DB...' -Level 'INFO'
                    try {
                    ...
                    try {
                        $UpdateTask = Update-TaskStatus -TicketNumber $TaskNumber -State '3' -WorkNotes 'Copilot license declined due to SOU/PTW training failure. Closing the task.'
                        Write-Log "Task $TaskNumber updated - Copilot license declined." -Level 'INFO'
                    }
                    catch {
                        $ex = $_.Exception.Message
                        Write-Log $ex -Level 'ERROR'
                        ScriptError('Failed to update task in snow at Copilot license declined..')
                    }

                    try {
                        $UpdateTicket = Update-TicketStatus -TicketNumber $RITMNumber -State '3' -Stage 'Completed' -WorkNotes 'Copilot license declined due to SOU/PTW training or TOU failure. Closing the ticket.'
                        Write-Log "RITM $RITMNumber updated - Copilot license declined." -Level 'INFO'
                    }
                    catch {
                        $ex = $_.Exception.Message
                        Write-Log $ex -Level 'ERROR'
                        Write-Log 'Failed to update ticket status at Copilot license declined..' -Level 'ERROR'
                    }
                    ...
                    ScriptError('Error while getting copilot license allocation function output.')
                    Write-Log "#####################################################################"
                    continue
                }
            }
            else {
                Write-Log "Not enough Copilot licenses available. Processing is on-hold..." -Level 'WARN'
                ...
                try {
                    Update-TicketStatus -TicketNumber $RITMNumber -Stage 'Waiting List' -State '5' -WorkNotes 'Copilot licenses are at capacity. Request will be processed once there are enough available licenses.'
                }
                catch {
                    $ex = $_.Exception.Message
                    Write-Log $ex -Level 'ERROR'
                ...
                Write-Log 'Updating EmailSentCount and Date in database...' -Level 'INFO'
